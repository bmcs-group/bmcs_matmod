...
if __name__ == "__main__":
    # When run as standalone script, add parent directories to path
    script_dir = Path(__file__).parent.absolute()
    gsm_lagrange_dir = script_dir.parent
    bmcs_matmod_dir = gsm_lagrange_dir.parent
    project_root = bmcs_matmod_dir.parent
...
# Import dependencies with fallback for standalone execution
try:
    # Try relative imports first (when used as module)
    from ..core.gsm_def_registry import (
    ...
    try:
        from bmcs_matmod.gsm_lagrange.core.gsm_def_registry import (
            get_available_gsm_defs, check_gsm_def_exists, 
            get_gsm_def_module_path, get_mechanism_description,
            get_gsm_def_class
        )
        from bmcs_matmod.gsm_lagrange.cli.data_structures import MaterialParameterData, LoadingData, SimulationConfig, SimulationResults
        from bmcs_matmod.gsm_lagrange.cli.parameter_loader import ParameterLoader
        from bmcs_matmod.gsm_lagrange.core.response_data import ResponseData
    except ImportError as e:
        print(f"Error: Could not import required modules: {e}")
        print("Make sure you're running from the project root or with proper PYTHONPATH")
        sys.exit(1)
        ...
        try:
            ...
            if hasattr(self.gsm_def_class, 'param_codenames'):
                ...
                for latex_symbol, codename in model_params.items():
                    ...
                    # Convert the latex_symbol (Scalar object) to string representation
                    latex_name = str(latex_symbol.name) if hasattr(latex_symbol, 'name') else str(latex_symbol)
