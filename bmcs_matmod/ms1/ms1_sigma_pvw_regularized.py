...
class MS1PVWEQ(MATS3DEval, InteractiveModel):
    ...
    def get_normal_law(self, eps_N_Emn, omega_N_Emn, z_N_Emn,
                       ...
                       plastic_dissip_N_Emn, damage_dissip_N_Emn, total_work_microplane, total_work_macro, eps_aux, eps_u,eps_b):
    ...
    # -------------------------------------------------------------------------
    def get_tangential_law(self, eps_T_Emna, omega_N_Emn, z_N_Emn,
                       alpha_N_Emn, r_N_Emn, eps_N_p_Emn, sigma_N_Emn, omega_T_Emn, z_T_Emn,
                       alpha_T_Emna, eps_T_pi_Emna, sigma_T_Emna, plastic_dissip_T_Emn, damage_dissip_T_Emn,
                       plastic_dissip_N_Emn, damage_dissip_N_Emn, total_work_microplane, total_work_macro, eps_aux, eps_u,eps_b):
    ...
    def get_corr_pred(self, eps_Emab, t_n1, **Eps):
        ...
        eps_aux = Eps.get('eps_aux')
        eps_b = Eps.get('eps_b')
        eps_u = Eps.get('eps_u')
        eps_aux = Eps.get('eps_aux')
        delta_eps = eps_Emab - eps_aux
        delta_eps_u = eps_Emab - eps_aux
        delta_eps_b = eps_Emab - eps_aux

        d_eps_u_Emab = np.zeros_like(eps_Emab)
        ...
        while k < k_max:
            Eps_k = copy.deepcopy(Eps)

            # d_eps_u_Emab = np.einsum('...ijkl,...kl->...ij', (self.L * self.h + self.L* self.h)/ (2 * self.L * self.L) \
            #                                    * D_abef,  r_Emab)

            # print(d_eps_u_Emab)

            delta_eps_u = delta_eps_u + d_eps_u_Emab


            delta_eps_b = (2 * self.L * self.L) / (self.L * self.h + self.L* self.h) * delta_eps + \
                          (2 * self.L * self.L - self.L* self.h - self.L* self.h)/ \
                          (self.L * self.h + self.L* self.h) * delta_eps_u

            eps_N_Emn = self._get_e_N_Emn(eps_b + delta_eps_b)
            eps_T_Emna = self._get_e_T_Emna(eps_b + delta_eps_b)
            ...
            sig_Emab_u = np.einsum('...ijkl,...kl->...ij', D_abef, eps_u + delta_eps_u)

            # print(delta_eps_u[0],sig_Emab_u[0],delta_eps_b[0],sig_Emab_b[0])

            r_Emab = sig_Emab_b - sig_Emab_u

            R_norm = np.einsum('...ij,...ij->...', r_Emab, r_Emab)
            R_norm = np.linalg.norm(R_norm)
            print(R_norm)
            ...
            d_eps_u_Emab = ((self.L * self.h + self.L * self.h) / (2 * self.L * self.L)) * 1 / self.E * \
                           ((1 + self.nu) * r_Emab - self.nu * np.einsum('...kk,...ij->...ij', r_Emab, delta))

            # d_eps_u_Emab = np.einsum('...ijkl,...kl->...ij', (self.L * self.h + self.L * self.h) / (2 * self.L * self.L) \
            #                          * D_abef, r_Emab)
            # print(np.einsum('...ij,...ij->...', d_eps_u_Emab, d_eps_u_Emab))
            # print(np.einsum('...ij,...ij->...', delta_eps, delta_eps))
            # R_norm = np.einsum('...ij,...ij->...', d_eps_u_Emab, d_eps_u_Emab) / np.einsum('...ij,...ij->...', delta_eps, delta_eps)
        ...
        eps_u[...] = eps_u + delta_eps_u

        eps_b[...] = eps_b + delta_eps_b

        eps_N_Emn = self._get_e_N_Emn(eps_b)
        eps_T_Emna = self._get_e_T_Emna(eps_b)
        ...
        beta_Emabcd = self._get_beta_Emabcd(eps_b, **Eps)
        ...
        D_Emabcd = np.einsum(
            '...ijab, abef, ...cdef->...ijcd',
            beta_Emabcd, D_abef, beta_Emabcd
        )

        total_work_micro, total_work_macro = self.get_total_work(eps_Emab, sig_Emab, **Eps)


        return sig_Emab, D_Emabcd


class MS12DPVWEQ(MS1PVWEQ):
